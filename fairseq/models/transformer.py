...
#        src_img_features = torch.from_numpy(src_img_features).cuda()
        ...
        ##########   生成随机的图片特征  ###############
#        src_img_features = (torch.randn(src_img_features.size(0),src_img_features.size(1),src_img_features.size(2))+1).cuda().half()

#        grid_img_mask = torch.sum(src_img_features,dim=-1).eq(1).transpose(0,1)

#        src_img_features = torch.zeros_like(src_img_features)



        
    ################  image to txt   ############    MultimodelMixup
        alpha = 0.2
...
#        encoder_mask_img = torch.zeros(x.size(1), batch_len).eq(1).cuda()
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                x = layer(x, src_img_features, encoder_padding_mask, batch_len, idx, alpha)
        ...
        if self.layer_norm is not None:
            ...
            if return_all_hiddens:
                encoder_states[-1] = x
##        encoder_padding_mask = torch.cat([encoder_padding_mask, encoder_mask_img], dim=-1)
        encoder_padding_mask = torch.cat([encoder_padding_mask, encoder_padding_mask], dim=-1)
...
def transformer_iwslt_de_en(args):
    ...
    args.mda_dim = getattr(args, 'mda_dim', 128)
    base_architecture(args)
